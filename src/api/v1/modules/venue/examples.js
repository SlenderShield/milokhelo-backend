...
async function createBasicBooking(venueService, venueId, userId) {
  ...
  try {
    const booking = await venueService.bookVenue(venueId, userId, {
...
async function smartBooking(venueService, venueId, userId, preferredTime) {
  ...
  const availability = await venueService.checkAvailability(venueId, preferredTime.date);
  ...
  const isSlotAvailable = availability.availableSlots.some(
    (slot) => slot.startTime === preferredTime.startTime && slot.endTime === preferredTime.endTime
  );
  ...
  try {
    const booking = await venueService.bookVenue(venueId, userId, preferredTime);
...
async function handleConcurrentBookings(venueService, venueId, userIds) {
  ...
  const bookingPromises = userIds.map((userId) =>
    venueService
      .bookVenue(venueId, userId, bookingData)
...
async function updateBookingTime(venueRepository, bookingId, newTime) {
  ...
  try {
    const updated = await venueRepository.updateBookingAtomic(bookingId, {
...
async function bookWithRetry(venueService, venueId, userId, bookingData, maxRetries = 3) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const booking = await venueService.bookVenue(venueId, userId, bookingData);
      ...
      if (error.message.includes('Invalid time format') || error.message.includes('duration')) {
...
async function checkMultipleDates(venueService, venueId, dates) {
  ...
  const availabilityPromises = dates.map((date) =>
    venueService.checkAvailability(venueId, date).then((availability) => ({ date, availability }))
  );
...
async function findAlternatives(venueService, venueId, preferredBooking, requiredDuration) {
  ...
  const availability = await venueService.checkAvailability(venueId, preferredBooking.date);
...
async function completeBookingFlow(venueService, venueId, userId) {
  ...
  try {
    ...
    const availability = await venueService.checkAvailability(venueId, bookingData.date);

    const isAvailable = availability.availableSlots.some(
      (slot) => slot.startTime === bookingData.startTime && slot.endTime === bookingData.endTime
    );
